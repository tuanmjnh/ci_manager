...
class membersType extends MX_Controller {
    ...
    public function index() {
        ...
        //Main value page
        $data['pTitle'] = LKEY::GET('membersTypePTitle');
        $data['mTitle'] = LKEY::GET('membersTypeList');
        //Sent data to View
    ...
    public function details_modal() {
        ...
        $data['d'] = $this->mdl_membersType->get_where($_POST['id']);
    ...
    public function data() {
        ...
        $TMPage = new TMPage($_POST['page'], $_POST['limit'], $this->mdl_membersType->get_count($where, $like));
        $thead = array(
        ...
        $tbody = array();
        $qry = $this->mdl_membersType->get_limit($where, $TMPage->get_limit(), $TMPage->get_offset(), $like);
        foreach ($qry as $row) {
            $tbody [] = array(
                "title" => $row->GVTitle,
                "order" => $row->GIOrder,
    ...
    public function insert() {
        ...
        try {
            ...
            $this->mdl_membersType->_insert($this->tmpluss->TrimArray($_POST), $images);
        } catch (Exception $e) {
            $rs = $e->getMessage();
        }
        echo json_encode($rs);
    ...
    public function Update() {
        ...
        try {
            ...
            if (($images = $this->tmpluss->upload_image(128, 128)) == 'error')
                throw new Exception(LKEY::GET('msgUploadError'));
            $this->mdl_membersType->_update($this->tmpluss->TrimArray($_POST), $images);
        } catch (Exception $e) {
            $rs = $e->getMessage();
        }
        echo json_encode($rs);
        ...
        try {
            if (is_array($_POST['id']))
                foreach ($_POST['id'] as $k => $v)
                    $this->mdl_membersType->_update_status($v, $_POST['flag']);
            else
                $this->mdl_membersType->_update_status($_POST['id'], $_POST['flag']);
        } catch (Exception $e) {
            $rs = $e->getMessage();
        }
        echo json_encode($rs);
    ...
    public function delete() {
        ...
        try {
            if (is_array($_POST['id']))
                foreach ($_POST['id'] as $k => $v)
                    $this->mdl_membersType->_delete($v);
            else
                $this->mdl_membersType->_delete($_POST['id']);
        } catch (Exception $e) {
            $rs = $e->getMessage();
        }
        echo json_encode($rs);

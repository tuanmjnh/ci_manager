...
//        echo Modules::run('template/content', $data);
    ...
    public function prints() {
        $data['_css'] = array(
            $this->template->AddAsset('css/bootstrap.min.css'),
        ...
        $data['_js'] = array(
            ...
            $this->template->AddAsset('Scripts/jquery.validate.js'),
        ...
        $this->load->view('prints_bills', $data);
    }

    public function prints_modal() {
        TMUrl::require_post();
        $data = $this->loadAssets();
        $data['mTitle'] = LKEY::GET('PrintBillsPTitle');
        $this->load->view('prints_modal', $data);
    }

    public function find_buyby_modal() {
        TMUrl::require_post();
        $data = $this->loadAssets();
        $data['mTitle'] = LKEY::GET('BillsFindBuyBy');
        $this->load->view('find_buyby_modal', $data);
    }
    ...
    public function data_products() {
        ...
        foreach ($qry as $row) {
            ...
            foreach ($this->mdl_product->get_where_sub($row->IUID) as $srow) {
                ...
                $select.="<li>$btnRadio <span><div>$lblColor: $srow->SIVEmail</div><div>$lblSize:$srow->SIVAuthor</div><div>$lblQty:$srow->SIVTitle</div></span></label></li>";
            }
            $select.=$li . '</ul></div>';
            $tbody [] = array(
                "Title" => $row->IVTitle,
...
//                . LKEY::GET('Choose') . '</label></div>'
    ...
    public function insert() {
        ...
        try {
            ...
            $total = $_POST['total'];
            $rs = $this->mdl_bills->_insert($members, $total, $products);
        } catch (Exception $e) {
            $rs = $e->getMessage();
        }
        echo json_encode($rs);
